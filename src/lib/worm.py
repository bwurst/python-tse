...
class Worm:
    ...
    def export_tar_incremental_ex(self, filename, lastState=None, maxExportSize=0, callback=None):
        ...
        with open(filename, 'wb') as self.tarfile:
            ...
            ret = self.wormlib.worm_export_tar_incremental_ex(self.ctx, last_state, last_state_len, new_state, new_state_len, maxExportSize, byref(allDataExported), byref(firstSignatureCounter), byref(lastSignatureCounter), callback, None)
            WormError_to_exception(ret)
